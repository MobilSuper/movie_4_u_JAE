...
reviews = []
for idx, url in enumerate(list_review_url[250:301]):
    driver.get(url)
    time.sleep(0.5)
    review = ''
    for i in range(1, 31):
...
df = pd.DataFrame({'titles': movie_titles[250:301], 'reviews': reviews})
today = datetime.datetime.now().strftime('%Y%m%d')
df.to_csv('./crawling_data/reviews_300.csv'.format(today), index=False)